...
# Copy server install assets into bundle (deploy/ is canonical source)
$deployRoot = Join-Path $PSScriptRoot "..\deploy"
Copy-Item (Join-Path $deployRoot "kube-prometheus-values-minimal.yaml") $OutDir -Force
$installSh = Join-Path $deployRoot "install-kube-prometheus-offline.sh"
...
if ($content -notmatch "DO NOT EDIT") {
    $header = "# DO NOT EDIT — copy from deploy/install-kube-prometheus-offline.sh via prepare-kube-prometheus-offline.ps1`n"
