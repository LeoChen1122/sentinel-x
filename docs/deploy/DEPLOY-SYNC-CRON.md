...
sudo bash /opt/sentinel-x/deploy/config/sentinel-config-apply.sh
```
...
| 改 namespace/容器 | `sudo bash deploy/config/sentinel-config-discover.sh --write && deploy/config/sentinel-config-apply.sh` |
...
| [`docs/deploy/DEPLOY-MCP-KUBECONFIG.md`](DEPLOY-MCP-KUBECONFIG.md) | W1-4 MCP kubeconfig |
