...
`mcp-servers/compose/docker-compose.yml` 把宿主机 `~/.kube` 只读挂到容器 `/root/.kube`。  
...
需确保 `mcp-servers/compose/docker-compose.yml` 中 `mcp-k8s` 含 `extra_hosts`（仓库模板已包含）。
...
若用 root 的 config，可设 `KUBECONFIG_HOST=/root/.kube` 或在 `mcp-servers/compose/.env` 中写一行。
...
/opt/sentinel-x/.venv/bin/python \
  /opt/sentinel-x/agents/langgraph-integration/scripts/live/mcp_k8s_sync_live.py
```
...
| 文件 | 作用 |
|------|------|
| [`mcp-servers/compose/docker-compose.yml`](../mcp-servers/compose/docker-compose.yml) | `extra_hosts`、`KUBECONFIG_HOST` 挂载 |
| [`deploy/sync-kubeconfig-for-mcp.sh`](../deploy/sync-kubeconfig-for-mcp.sh) | 复制 k3s.yaml 并重写 server URL |
| [`docs/deploy/DEPLOY-SYNC-CRON.md`](DEPLOY-SYNC-CRON.md) | W1-3 cron（依赖本 kubeconfig） |
| [`docs/deploy/DEPLOY-LANGGRAPH-SYSTEMD.md`](DEPLOY-LANGGRAPH-SYSTEMD.md) | W1-2 LangGraph 常驻 |
