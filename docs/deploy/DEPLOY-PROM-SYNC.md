...
compose 默认（可被 `mcp-servers/compose/.env` 覆盖；kube-prometheus 离线安装后应为 **30909**）：
...
# 或: sudo bash deploy/config/sentinel-config-apply.sh --with-prom-sync

sudo /usr/local/bin/sentinel-sync-prom.sh
# 或:
# python /opt/sentinel-x/agents/langgraph-integration/scripts/live/mcp_prom_sync_live.py
```
...
# top_pods_by_cpu / pod_metrics — 见 agents/langgraph-integration/scripts/demo/query_demo.py --live
```
...
| `agents/langgraph-integration/scripts/live/mcp_prom_sync_live.py` | 入口脚本 |
