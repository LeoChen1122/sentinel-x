...
MCP_ENV="${ROOT}/mcp-servers/compose/.env"
...
configure_mcp_prometheus_url() {
  ...
  if [[ -d "${ROOT}/mcp-servers/compose" ]]; then
    (cd "${ROOT}/mcp-servers/compose" && docker-compose up -d mcp-prometheus) || true
    local prom_c
...
Next (W3):
  ...
  python ${ROOT}/agents/langgraph-integration/scripts/live/mcp_prom_sync_live.py

See: ${ROOT}/docs/deploy/DEPLOY-PROM-SYNC.md
EOF
}
